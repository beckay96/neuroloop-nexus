...
export function EmergencyButton({ userId, className = "" }: EmergencyButtonProps) {
  ...
  const textEmergencyContact = () => {
    if (!emergencyContact?.phone || !location) {
      toast({
        ...
        description: location ? "No emergency contact found" : "Location not available yet",
        variant: "destructive"
      });
      return;
    }
  ...
  const logEmergencyEvent = async (eventType: string, details: any) => {
    try {
      // Use RPC to log emergency events to private_health_info.tracking_entries
      const { error } = await supabase.rpc('log_emergency_event', {
        p_user_id: userId,
      ...
      if (error) {
        ...
        // Fallback: just log to console if RPC fails
        console.log(`📝 Emergency Event: ${eventType}`, details);
      }
    } catch (error) {
              ...
              {emergencyContact && (
                <div className="space-y-2">
                  ...
                  <Button
                    size="lg"
                    variant="outline"
                    onClick={textEmergencyContact}
                    disabled={!location}
                    className="w-full h-12 text-base border-2 border-blue-500 text-blue-600 hover:bg-blue-50 disabled:opacity-50"
                  >
                    <MessageSquare className="h-5 w-5 mr-2" />
                    Text My Location
                    {!location && <span className="ml-2 text-xs">(Getting location...)</span>}
                  </Button>
...
export function FloatingEmergencyButton({ userId }: { userId: string }) {
  ...
  const callEmergencyServices = () => {
    const emergencyNumber = getEmergencyNumber('AU');
    window.location.href = `tel:${emergencyNumber}`;
    setActionsTaken([...actionsTaken, `Called ${emergencyNumber}`]);
    logEmergencyEvent('emergency_services_called', { number: emergencyNumber });
  };
